...
c You can turn off (jf( )=.false.) the computation of certain parameters 
...
C 2020.06 12/10/24 Ne-topside: COR2 default
C 2020.06 12/10/24 Including output for Sporadic-E Occ. Prob. oar(91,*)
C
      INTEGER           pad1(6),jdprof(77),piktab
      DIMENSION         outf(20,1000),oar(100,1000),jfi(6)
      LOGICAL		    jf(50),rzino,igino
      CHARACTER*2       timev(2)
      CHARACTER*3       uni(91),sopt,seopt
...
1       print *,'jmag(=0/1,geog/geom),lati/deg,long/deg'
     ...
     &          ' (valid range: 65-30000 km) (0,0 for no TEC)'
...
c          jf(8)=.false.      ! t=foF2	f=user input (t)
c          jf(9)=.false.      ! t=tops f10.7<188 f=unlimited (t)
c          jf(10)=.false.     ! t=Te standard f=using Ne user input (t)
c          jf(11)=.false.     ! t=Ne standard f=Lay-function (t)
c          jf(12)=.false.     ! t=messages to unit 6 f=messages.txt on 11 (t)
c          jf(13)=.false.     ! t=foF1 f=foF1 or NmF1 user input (t)
c          jf(14)=.false.     ! t=hmF1 f=user input (only Lay version) (t)
c          jf(15)=.false.     ! t=foE f= foE or NmE user input (t)
c          jf(16)=.false.     ! t=hmE f=hmE user input (t)
c          jf(17)=.false.     ! t=hmF1 f=user input (only Lay version) (t)
c          jf(18)=.false.     ! t=IGRF dip,magbr,modip f=POGO68/10 for 1973(t)
...
c          jf(21)=.false.     ! t=ion drift computed f=not comp.(t)
...
c          jf(29)=.false.     ! t=old  f=New Topside options (t)
...
c          jf(33)=.false.     ! t=auroral boundary   f=off (f)
...
c          jf(45)=.false.     ! t= Es occ. probability, f= off (t)
c          jf(46)=.false.     ! t= Es occ w/o solar/mag depend (t)
...
c          jf(48)=.false.     ! t=Ti-Tru2021 f=Ti-Bil1981 (t)
          ...
          if(jf(1)) then
c              print *,'Ne lower boundary: t=65/80km day/night, ',
c     &              'f=user input {t}'
c              	read(5,*) jf(45)
c              print *,'Ne upper boundary: t=2000km day/night, ',
c     &              'f=user input {t}'
c              	read(5,*) jf(46)
              print *,'Sporadic E Occ. Prob.: t=on f=off {t}'
              	read(5,*) jf(45)
              print *,'LAY version: t=standard ver., f=LAY version.',
     &              ' {standard:t}'
              	read(5,*) jf(11)
              print *,'Ne Topside: jf(29),jf(30)                  {t,f}'
     ...
     &              'f=user input {t}'
              ...
              if(jf(9)) then
                print *,'hmF2: t=f(M3000F2), f=new models {f}'
                	read(5,*) jf(39)              
                print *,'hmF2: t=AMTB-model, f=Shubin-COSMIC model {f}'
                	read(5,*) jf(40)              
                if(jf(39)) then
...
c
      ...
      if(PIKTAB.eq.3) then
        ...
        print *,'      sporadic-E probability [91]' 
        print *,'      CGM-Lat of equatorw. auroral boundary [58]'
...
7234    format('CGM coordinate computation is turned off') 
7235    format('Auroral boundary computation is turned off')
...
c ----------- SZA, Ne ----------------
c        WRITE(8,4919) oar(23,li),outf(1,li)
c4919    FORMAT(F7.1,2X,E12.5)
c ----------- Ne, TEC ----------------
...
c        WRITE(8,4919) XCOR,oar(91,li)
c4919    FORMAT(F7.1,2X,F6.1)
c ----------- CGM Coordinates ---------
        WRITE(8,4919) XCOR,(oar(jk,li),jk=54,62),
     &     oar(54,li)-oar(57,li)
4919    FORMAT(F7.1,2X,9F6.1)
